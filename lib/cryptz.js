...
function HASH_hascSalt(chash_plain, saltz){
    ...
    var chash_1 = crypto.createHash("sha1").update(chash_plain + "?:"+saltz).digest("hex");
    ...
    var chash_100 = crypto.createHash("sha512").update(chash_99).digest("hex");

    var encSaltz = crypto.createHash("sha512").update(saltz).digest("hex");
    return chash_100+"F"+encSaltz;
}
    ...
    "2FA_Purposes": {
        baseDATOS: {
            enc: function (datos){
                ...
                var datos_1 = Buffer.from(datos, "base64").toString("ascii");// 1
                var datos_2 = Buffer.from(datos_1, "base64").toString("binary");// 2
                var datos_3 = Buffer.from(datos_2, "binary").toString("hex");//3
                var datos_4 = Buffer.from(datos_3, "hex").toString("ascii");//4
                var datos_5 = Buffer.from(datos_4, "base64").toString("ascii");//5
                return datos_5;
            },
            dec: function (datos){
                
                var datos_1 = Buffer.from(datos, "base64").toString("ascii");//5
                var datos_2 = Buffer.from(datos_1, "hex").toString("ascii");//4
                var datos_3 = Buffer.from(datos_2, "binary").toString("hex");//3
                var datos_4 = Buffer.from(datos_3, "base64").toString("binary");// 2
                var datos_5 = Buffer.from(datos_4, "base64").toString("ascii");// 1
                return datos_5;
            },
        },
    },
